...
class WpAutomaticFacebook extends wp_automatic
{
    ...
    public function fb_get_post($camp)
    {
        ...
        if (wp_automatic_trim($cg_fb_page_id) == '') {
             try {
                $cg_fb_page_id = $this->get_page_id_from_url($url);
            ...
            if (1) {
                ...
                if ($cg_fb_from != 'events') {
                    ...
                    if($cg_fb_source == 'profile' && count($items) == 0){
                        ...
                        if(isset($matches[1]) && count($matches[1]) > 0){
                            ...
                            //$post_ids= array('1239242644872060');
                ...
                foreach ($items as $item) {
                    ...
                    echo '<br>Redirected to:' . $redirec_url;
                                       ...
                                       //$exec2 = preg_replace('!__isFeedUnit":"Story",.*?script>!s', '', $exec2,-1,$count);

                                       //report count
                                       //echo '<br>Removed '.$count.' timeline stories';
                   
                     //remove timeline stories starting from {"data":{"viewer":{"news_feed"
                     $exec2 = $this->remove_timeline_stories($exec2);  
    ...
    public function is_fb_not_logged_in($url, $status_code,$exec='')
    {
        ...
        //.facebook.com/login/
        if (stristr($url, 'facebook.com/login/')) {
            return true;
        }
    ...
    //function remove_timeline_stories
    function remove_timeline_stories($exec2)
    {
       
        echo '<br>Checking if the content contains timeline stories';

        //check if contains {"data":{"viewer":{"news_feed"
        if (stristr($exec2, '{"data":{"viewer":{"news_feed')) {
        
            echo '<-- Content contains timeline stories... removing';
            
            //remove timeline stories
            $exec2_parts = explode ('{"data":{"viewer":{"news_feed', $exec2);
            $exec2 = $exec2_parts[0];
           
        
        
        }else{
            echo '<-- Content does not contain timeline stories';
        }
    ...
    //function to get the page ID from a given URL 
    function get_page_id_from_url($url)
    {
        $page_id = '';

         echo '<br>Getting page id from url';
                        
                        //building the load URL on this form https://www.facebook.com/profile.php?id=100063762515341
                         

                        //url encode
                        $cg_fb_page_feed2 = urlencode($url);

                        $curl_url = 'https://www.facebook.com/platform/plugin/tab/renderer/?key=timeline&config_json=%7B%22app_id%22%3A%222012669838949787%22%2C%22href%22%3A%22'.$cg_fb_page_feed2 .'%22%2C%22width%22%3A340%2C%22height%22%3A500%2C%22has_cta%22%3Atrue%2C%22has_small_header%22%3Afalse%2C%22has_adapt_container_width%22%3Atrue%2C%22has_cover%22%3Atrue%2C%22has_posts%22%3Afalse%2C%22tabs%22%3A%22timeline%22%2C%22can_personalize%22%3Afalse%2C%22is_xfbml%22%3Atrue%2C%22referer_uri%22%3A%22%22%7D&__a=1&__req=1';
                        ...
                        if (stristr($exec, '"profileID":"')) {
                            preg_match('{"profileID":"(.*?)"}', $exec, $page_id_matches);
                            if (isset($page_id_matches[1]) && wp_automatic_trim($page_id_matches[1]) != '') {
                                $page_id = $page_id_matches[1];

                                //return the page id
                                echo '<br>Page ID found:' . $page_id;
                                return $page_id;
                            }
                        }
                        ...
                        //throw exec
                        if ($cu_code != 200) {
                            throw new Exception('Facebook API request failed with code ' . $cu_code);
                        }

                        throw new Exception('Can not extract page ID from the given URL');
    }
