...
class wp_automatic_amazon_api_less
{
    ...
    public function getItemByAsin($asin_code, $slug = '')
    {
        ...
        // Check for captcha if exists "errors/validateCaptcha" and throw an error
        if (stristr($exec, 'errors/validateCaptcha')) {
            
            //error unfortunately, amazon returned a captcha page instead of the product page, you will need to use a private IP on the plugin settings page to bypass this issue
            throw new Exception('Amazon returned a captcha page instead of the product page, you will need to use a private IP on the plugin settings page to bypass this issue');
        ...
        //name="items[0.base][customerVisiblePrice][displayString]" value="$6.49"
        }elseif ( preg_match('{name="items\[0.base\]\[customerVisiblePrice\]\[displayString\]" value="([^<]*?)"}s', $exec, $possible_price_matches) ) {

            echo '<br>Price tag:items[0.base][customerVisiblePrice][displayString]';

            preg_match_all('{name="items\[0.base\]\[customerVisiblePrice\]\[displayString\]" value="([^<]*?)"}s', $exec, $possible_price_matches);
            $possible_price_matches = $possible_price_matches[1];
            ...
            //this one may return incorrect results, for example it returns the warrenty price for a macbook B0DZDC3WW5
        ...
        //identify dicimal separator <span class="a-price-decimal">.</span>
        echo '<br>Identifying decimal separator from the source amazon site...';
        $decimal_separator = '';
        if (stristr($exec, '<span class="a-price-decimal">')) {
            preg_match('{<span class="a-price-decimal">(.*?)</span>}', $exec, $possible_decimal_matches);
            if (isset($possible_decimal_matches[1]) && wp_automatic_trim($possible_decimal_matches[1]) != '') {
                echo '<--Decimal separator found:' . $possible_decimal_matches[1];
                $decimal_separator = $possible_decimal_matches[1];
            }else{
                echo '<--Decimal separator not found in HTML';
            }
        }else {
            echo '<--HTML for Decimal separator not found';
        }

        
        //default decimal separator
        $item_pre_price = $item_price;//initialize pre price
        $item_price = $this->adjustPrice($item_price, $decimal_separator);
        ...
        echo '<br>Item pre price :' . $item_pre_price;

        //adjust pre price
        $item_pre_price = $this->adjustPrice($item_pre_price, $decimal_separator);
        ...
        if ($elements->length > 0) {
            ...
            $brand_array = explode(' ', trim($item_brand));
    ...
    public function getASINs($moreUrl, $htmlAdded = false)
    {
        ...
        if (count($asins) == 0) {
            ...
            foreach ($elements as $single_asin_element) {
                ...
                //<span class="a-price-whole">21<span class="a-price-decimal">.</span></span>
    ...
    private function adjustPrice($item_price, $decimal_separator )
    {

        echo '<br>Adjusting price...';

        //if decimal seprator is empty, return the price as is
        if ($decimal_separator == '') {
            echo '<-- No decimal separator found, returning price as is: ' . $item_price;
            return $item_price;
        }

        //desired decimal seprator wp_automatic_amazon_price_decimal
        $desired_decimal_separator = trim(get_option('wp_automatic_amazon_price_decimal', '')  );
        $wp_automatic_amazon_price_thousands = get_option('wp_automatic_amazon_price_thousands','');

        //if desired decimal seprator is not empty 
        if ($desired_decimal_separator != '') {

            //replace the decimal separator on the price with !
            $item_price = str_replace($decimal_separator, '!', $item_price);
            
            //replace . and , and spcace with the thousands seprator ,U+202f
            $item_price = str_replace(array('.', ',', ' ',' '), $wp_automatic_amazon_price_thousands, $item_price);
            
            //replace ! with the desired decimal separator
            $item_price = str_replace('!', $desired_decimal_separator, $item_price);

            echo '<-- Adjusted to: ' . $item_price ;
            

        }else{
            echo '<-- No desired decimal separator set in the plugin settings page , returning price as is: ' . $item_price;
        }
