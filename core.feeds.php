...
class WpAutomaticFeeds extends wp_automatic
{
    public function feeds_get_post($camp)
    {
        ...
        //if OPT_RANDOM_FEEDS is set then randomize the feeds array
        if (in_array('OPT_RANDOM_FEEDS', $this->camp_opt)) {
            echo '<br>Randomizing feeds...';
            shuffle($feeds);
        }
    ...
    public function feed_process_link($feed, $camp)
    {
        ...
        //correct Google trends links to https://trends.google.com/trending/rss?geo=US
        if (stristr($feed, 'trends.google.com')  ) {

            //echo
            echo '<br>Google trends feed found, correcting the link...';

            //find geo parameter
            preg_match('{geo=(.*?)[&]}', $feed, $geoMatches);
            $geo = isset($geoMatches[1]) ? $geoMatches[1] : '';

            echo '<br>geo parameter found: ' . $geo;

             
            //if geo is not set then set it to US
             if (wp_automatic_trim($geo) == '') {
                $geo = 'US';
                echo '<br>geo parameter not found, setting it to US';
            }

            //build from  https://trends.google.com/trending/rss?geo=US
            $feed = 'https://trends.google.com/trending/rss?geo=' . $geo;

            echo '<br>Google trends feed corrected to: ' . $feed;
        ...
        //replace for= by q=
        if (stristr($feed, 'news.google.com') && stristr($feed, 'for=')) {
            $feed = str_replace('for=', 'q=', $feed);
            echo '<br>Google news link corrected to: ' . $feed;
        }
        ...
        $camp_options = $camp_opt = unserialize($camp->camp_options);
        ...
        if ($this->isProxified && !(stristr($feed, '.xml') && stristr($feed, 'wp-content'))) {
            ...
            $md5 = md5($feed);
            $downloadedFileUrl = $this->download_file($feed, $md5 . '.html');
        ...
        // if feed link contains trends.google convert the raw data to a simple RSS format
        if (stristr($feed, 'trends.google') && isset($rss->raw_data)) {
            
            // echo feed raw data
            echo '<br>Google trends feed found, converting the raw data to a simple RSS format...';
             
            try {

                //convert
                $raw_data = $this->convert_google_trends($rss->raw_data);
        ...
        if (!$this->is_duplicate($url)) {
            ...
            if (in_array('OPT_FULL_FEED', $camp_opt) || in_array('OPT_FEED_CUSTOM', $camp_opt) || in_array('OPT_FEED_CUSTOM_R', $camp_opt) || in_array('OPT_ORIGINAL_META', $camp_opt) || in_array('OPT_ORIGINAL_CATS', $camp_opt) || in_array('OPT_ORIGINAL_TAGS', $camp_opt) || in_array('OPT_ORIGINAL_AUTHOR', $camp_opt) || in_array('OPT_FEED_PTF', $camp_opt) || in_array('OPT_FEEDS_OG_IMG', $camp_opt) || $og_title_enabled) {
                ...
                // strip parts using css id/class
                // Stripping content using id or class from $res[cont]
                if (in_array('OPT_STRIP_CSS', $camp_opt)) {

                    //flag for already processed strip css for not to process them again if we got a full content
                    $already_processed_strip_css = true;
                    ...
                    $i = 0;
                    $inner = false;
                    $cg_feed_visual_strip_regex = array(); //ini xpaths to remove by
                    foreach ($cg_selecotr_data as $cg_selector_data_single) {
                        ...
                        if (wp_automatic_trim($cg_selector_data_single) != '') {
                            ...
                            if ($cg_selector[$i] == 'class') {
                             ...
                             $cg_feed_visual_strip_regex[] = $query_final; 
                        }

                        $i++;
                    }

                    $cg_feed_visual_strip = array_filter($cg_feed_visual_strip_regex);
            ...
            if (in_array('OPT_STRIP_CSS', $camp_opt) && !isset($already_processed_strip_css)) {
            ...
            if (! in_array('OPT_MIN_LENGTH', $camp_opt) && isset($camp->camp_sub_type) && ($camp->camp_sub_type == 'BingNews' || $camp->camp_sub_type == 'GoogleNews'  || $camp->camp_sub_type == 'GoogleTrends')) {
 ...
 */
    ...
    //function convert_google_trends to extract items from google trends rss feed and convert it to a simple format
    ...
    <item>
			...
			<pubDate>Thu, 1 May 2025 09:30:00 -0700</pubDate>
			<ht:picture>https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcQoFo4892PQHusRewnQW9rWqT03RiHJlDazlhYNEiZt6z2USP432GNzk4ndhAU</ht:picture>
			<ht:picture_source>Seeking Alpha</ht:picture_source>
			<ht:news_item>
				<ht:news_item_title>Organon stock drops after dividend cut (OGN:NYSE)</ht:news_item_title>
				<ht:news_item_snippet/>
				<ht:news_item_url>https://seekingalpha.com/news/4438939-organon-stock-drops-dividend-cut</ht:news_item_url>
				<ht:news_item_picture>https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcQoFo4892PQHusRewnQW9rWqT03RiHJlDazlhYNEiZt6z2USP432GNzk4ndhAU</ht:news_item_picture>
				<ht:news_item_source>Seeking Alpha</ht:news_item_source>
			</ht:news_item>
			<ht:news_item>
				<ht:news_item_title>Organon (NYSE:OGN) Posts Better-Than-Expected Sales In Q1 But Stock Drops 23.5%</ht:news_item_title>
				<ht:news_item_snippet/>
				<ht:news_item_url>https://finance.yahoo.com/news/organon-nyse-ogn-posts-better-140426516.html</ht:news_item_url>
				<ht:news_item_picture>https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTrWV67VlEu5xBmpuM4ea_v7fU51N3zYIh5BMTOSbwSeEYU3CGFDoLuV_vvISI</ht:news_item_picture>
				<ht:news_item_source>Yahoo Finance</ht:news_item_source>
			</ht:news_item>
			<ht:news_item>
				<ht:news_item_title>Organon &amp; Co reports results for the quarter ended March 31 - Earnings Summary</ht:news_item_title>
				<ht:news_item_snippet/>
				<ht:news_item_url>https://www.tradingview.com/news/reuters.com,2025:newsml_L8N3R91CS:0-organon-co-reports-results-for-the-quarter-ended-march-31-earnings-summary/</ht:news_item_url>
				<ht:news_item_picture>https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcQJyFOEBDlFEn6RCn37vHVe0wcGXcDhfzRauRvVOacGC_r81GINrLHKN7sNWFg</ht:news_item_picture>
				<ht:news_item_source>TradingView</ht:news_item_source>
			</ht:news_item>
		</item>
    */
    
    function convert_google_trends($original_cont){

       // echo '<br>Converting Google Trends RSS feed from:';
        //echo $original_cont;
        //exit;

        //if orignal content is not a string and contains "<title>" throw exception
        if (!is_string($original_cont) || !stristr($original_cont, '<title>')) {
        ...
        if ($items->length == 0) {
            throw new Exception('No items found in the content');
        }
        ...
        foreach ($items as $item) {
            ...
            //get first news item <news_item>...</news_item> using REGEX
            preg_match('!<news_item>(.*?)</news_item>!s', $item_html, $news_items);

            //remove news_item_ text from the result using str_replace
            $news_items[1] = str_replace('news_item_', '', $news_items[1]);

            //wrap using <item>...</item>
            $news_items[1] = '<item>' . $news_items[1] . '</item>';

            //replace <url > with <link>
            $news_items[1] = str_replace('<url>', '<link>', $news_items[1]);
            $news_items[1] = str_replace('</url>', '</link>', $news_items[1]);


            //add to results 
            $results[] = $news_items[1];
        ...
        //if count is 0 throw exception
        if (count($results) == 0) {
            throw new Exception('No items found in the content results arr');
        }

        //build the final feed from the results
        /*single item HTML

         <item>
				<title>PSL 2025: Lahore vs Quetta playing 11, live toss, streaming details</title>
				<snippet></snippet>
				<url>https://www.business-standard.com/cricket/news/psl-2025-lahore-vs-quetta-playing-11-live-toss-streaming-details-125043001684_1.html</url>
				<picture>https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcQT1R1x9Qoq-OjtKoHfgsxGHvq6zHuLWCR7r63cDTYvK6yjMrxmTn07dRYEu2Y</picture>
				<source>Business Standard</source>
			</item>

        */

        $final_feed = '<rss version="2.0"><channel><title>Google Trends</title><link>https://trends.google.com/trending/rss?geo=US</link><description>Google Trends</description>';
        foreach ($results as $result) {
            $final_feed .= $result;
        }
        $final_feed .= '</channel></rss>';
